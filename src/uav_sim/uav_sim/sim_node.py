...
from uav_interfaces.msg import BatteryState, RTHDecision, UavState, WindState


class UAVSimNode(Node):
    """Lightweight UAV simulator with battery drain, wind disturbance and RTH execution."""
    ...
    def rth_callback(self, msg):
        """Switch from mission execution to return-to-home when risk becomes unacceptable."""
        if msg.trigger_rth and self.mode == 'MISSION':
            self.mode = 'RTH'
            self.get_logger().warn(
                'RTH activated: '
                f'P_safe={msg.safe_return_probability:.3f}, '
                f'threshold={msg.risk_threshold:.2f}'
            )

    def update(self):
        dt = 0.1

        wind_speed = max(0.0, 3.0 + random.gauss(0.0, 0.4))
        ...
        if self.mode == 'MISSION':
            ...
            if dist > 0.1:
                self.x += self.rth_speed * dx / dist * dt
                self.y += self.rth_speed * dy / dist * dt
            else:
                self.x = self.home_x
                self.y = self.home_y
                self.vx = 0.0
                ...
                if not self.has_landed:
                    ...
                    self.get_logger().info('UAV safely returned home and landed.')
